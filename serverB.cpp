...
int main()
{
    ...
    */
    ...
    int udp_socket;
    struct sockaddr_in udp_server_B_addr, udp_server_main_addr;
    char recv[MAX];
    ...
    udp_server_B_addr.sin_addr.s_addr = inet_addr(localhost);

    if ( bind(udp_socket_B, (const struct sockaddr *)&udp_server_B_addr,  
            sizeof(udp_server_B_addr)) < 0 ) 
        { 
            perror("bind failed"); 
            exit(EXIT_FAILURE); 
        }
    ...
    int recv_m;
    recv_m = read(udp_socket_B,recv,1024);
    recv[recv_m] = '\0';
    //code was copied and ajusted from geeks for geeks https://www.geeksforgeeks.org/udp-server-client-implementation-c/?ref=rp
